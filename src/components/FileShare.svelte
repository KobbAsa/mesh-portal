<script>
    ...
    async function handleFileUpload(event) {
        ...
        try {
            ...
            alert("Failed to upload file to the mesh.");
        } finally {
            isUploading = false;
        }
    }
...
<section class="files-area">
    ...
    <div class="dropzone">
        ...
        <button class="upload-btn" on:click={() => fileInput.click()} disabled={isUploading}>
            {isUploading ? 'Transmitting...' : 'Select file from disk'}
        </button>
    </div>
        ...
        {#each files as file}
          ...
          <span class="name" title={file.name}>
            {file.name.length > 20 ? file.name.substring(0, 20) + '...' : file.name}
                ...
                <a href={`http://${serverIP}:3000${file.url}`} target="_blank" class="download-btn" download>⬇️</a>
            </div>
        {/each}

        {#if files.length === 0}
            <p class="empty-state">No files available. Awaiting data...</p>
        {/if}
    </div>
</section>
